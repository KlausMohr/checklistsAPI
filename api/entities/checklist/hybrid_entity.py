...
class Hybrid(db.Model):
    ...
    hybrid_cooling_system = db.Column(db.Integer(), nullable=False)
    switchable_powertrain_mount = db.Column(db.Integer(), nullable=False)
    hybrid_entertainment_information_display = db.Column(
        db.Integer(), nullable=False)
    power_outlet_low_voltage = db.Column(db.Integer(), nullable=False)
    created_at = db.Column(
        TIMESTAMP(timezone=True), nullable=False, server_default=text("now()")
    )

    def __repr__(self):
        return f"Hybrid Systema [id = {self.id},
