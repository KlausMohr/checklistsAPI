            ...
            try:
                ...
                for checklist, vehicle, owner in data:
                    obj = {"checklist": checklist.to_json(),
                           "vehicle": {"model": vehicle.model, "licenseplt": vehicle.licenseplt},
                           "owner": owner.name}
                    response.append(obj)

                return response_gen(200, "Checklist Invoice", response, "Lista de checklists realizados")
            except:
                response = {}
                return response_gen(204, "No content found for Checklists Invoice", response)
